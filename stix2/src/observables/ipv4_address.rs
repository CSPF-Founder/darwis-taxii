...
pub struct IPv4Address {
    ...
    /// Specifies a list of references to MAC addresses that the IPv4 address resolves to.
    /// References must be of type `mac-addr`.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub resolves_to_refs: Vec<Identifier>,

    /// Specifies a list of references to Autonomous Systems that the IPv4 address belongs to.
...
impl IPv4Address {
    /// The STIX type identifier for IPv4 Address.
    pub const TYPE: &'static str = "ipv4-addr";

    /// Create a new IPv4 Address with a deterministic ID based on the value.
    ///
    /// # Arguments
    ///
    /// * `value` - The IPv4 address value (e.g., "192.168.1.1")
    ///
    /// # Example
    ///
    /// ```rust,ignore
    ...
    /// Create a new defanged IPv4 Address.
    ...
    /// Add an Autonomous System reference that this IPv4 address belongs to.
    ///
...
mod tests {
    ...
    fn test_create_ipv4() {
        ...
        assert_eq!(ip.type_, "ipv4-addr");
        assert!(ip.id.to_string().starts_with("ipv4-addr--"));
    }

    #[test]
    fn test_deterministic_id() {
        let ip1 = IPv4Address::new("192.168.1.1").unwrap();
        let ip2 = IPv4Address::new("192.168.1.1").unwrap();
    ...
    fn test_different_values_different_ids() {
        let ip1 = IPv4Address::new("192.168.1.1").unwrap();
        let ip2 = IPv4Address::new("192.168.1.2").unwrap();
    ...
    #[test]
    fn test_defanged() {
        let ip = IPv4Address::defanged("192.168.1.1").unwrap();
        assert!(ip.defanged);
    }
    ...
    fn test_resolves_to_refs_validation() {
        ...
        let bad_ref: Identifier = "ipv6-addr--12345678-1234-1234-1234-123456789abc"
            .parse()
            .unwrap();
    ...
    fn test_serialization() {
        let ip = IPv4Address::new("192.168.1.1").unwrap();
        let json = serde_json::to_string(&ip).unwrap();
        assert!(json.contains("\"type\":\"ipv4-addr\""));
        assert!(json.contains("\"value\":\"192.168.1.1\""));

        // Deserialize and verify
        let parsed: IPv4Address = serde_json::from_str(&json).unwrap();
